...
- **Basic auth is always on.** This module is built for deployments where workspaces share a network namespace (`network_mode = "container:..."`), where `127.0.0.1` is reachable from every sibling workspace. A per-workspace random credential (`random_password`) gates the VNC endpoint; a random port alone would be obscurity, not authentication. The workspace owner finds the password in `$CODER_WPILIB_SIM_PASSWORD` (VNC username `coder`) and enters it once per browser session.
